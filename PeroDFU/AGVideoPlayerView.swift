...
extension AGVideoPlayerView {
    ...
    fileprivate func play() {
        if isPlaying { return }
        isPlaying = true
        videoAsset?.loadValuesAsynchronously(forKeys: ["playable", "tracks", "duration"], completionHandler: <#T##(() -> Void)?##(() -> Void)?##() -> Void#>)(forKeys: <#T##[String]#>, completionHandler: <#T##(() -> Void)?##(() -> Void)?##() -> Void#>).loadValuesAsynchronously(forKeys: ["playable", "tracks", "duration"], completionHandler: { [weak self] _ in
    ...
    @objc fileprivate func itemDidFinishPlaying() {
        if isPlaying {
            playerController.player?.seek(to: CMTime.zero, toleranceBefore: kCMTimeZero, toleranceAfter: kCMTimeZero)
